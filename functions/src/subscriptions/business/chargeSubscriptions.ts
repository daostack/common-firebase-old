...
export const chargeSubscriptions = async (): Promise<void> => {
  ...
  const subscriptionsDueToday = (await db
    .collection(Collections.Subscriptions)
    // .where('dueDate', '>=', new Date().setHours(0,0,0,0))
    .where(
      'dueDate',
      '<=',
      Timestamp.fromMillis(new Date().setHours(23, 59, 59, 999)),
    )
  ...
  for (const subscriptionDueToday of subscriptionsDueToday.docs) {
    ...
    if (
      ...
      promiseArr.push(
        (async () => {
          logger.info(
            `Charging subscription (${subscriptionEntity.id}) with $${subscriptionEntity.amount}`,
            {
