...
export const createSubscription = async (
  ...
  if (proposal.type !== 'join') {
    throw new CommonError(
      'Cannot create subscription for proposals that are not join proposals',
      {
        proposal,
      },
    );
  ...
  if (await subscriptionDb.exists({proposalId: proposal.id})) {
    throw new CommonError(
      'There is already created subscription for this proposal',
      {
        proposal,
      },
    );
  ...
  if (isSuccessful(payment)) {
    ...
    logger.warn('Initial subscription payment for subscription failed!', {
      subscription,
      payment,
    });
