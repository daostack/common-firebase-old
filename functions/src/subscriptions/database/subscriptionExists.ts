...
export const subscriptionExists = async (
  args: ISubscriptionExistsArgs,
): Promise<boolean> => {
  let subscription: DocumentSnapshot<ISubscriptionEntity>;

  if (args.id) {
    subscription = (await SubscriptionsCollection.doc(
      args.id,
    ).get()) as DocumentSnapshot<ISubscriptionEntity>;
  }

  if (args.proposalId) {
    const where = await SubscriptionsCollection.where(
      'proposalId',
      '==',
      args.proposalId,
    ).get();
