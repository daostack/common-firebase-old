...
const processNotification = async (notification: INotificationModel) => {
  ...
  if (!currNotifyObj.data) {
    throw Error(
      `Not found data method for notification on event type "${notification.eventType}".`,
    );
  }
  ...
  if (notification.userFilter) {
    notification.userFilter.forEach(async (filterUserId) => {
      ...
      if (currNotifyObj.notification) {
        try {
          const {title, body, image, path} = await currNotifyObj.notification(
            eventNotifyData,
          );
  ...
  if (currNotifyObj.email) {
    ...
    const emailTemplateArr = Array.isArray(emailTemplate)
      ? emailTemplate
      : [emailTemplate];
...
exports.commonNotificationListener = functions.firestore
