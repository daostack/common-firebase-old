...
export const notifyData: Record<string, IEventData> = {
      ...
      return [
        ...
        },
        {
          ...
          emailStubs: {
            ...
            minContribution: (
              commonData.metadata.minFeeToJoin / 100
  ...
  [EVENT_TYPES.FUNDING_REQUEST_CREATED]: {
    ...
    notification: async ({proposalData, commonData, userData}) => {
      return {
        ...
        body: `Your fellow member ${getNameString(userData)} is asking for $${
          proposalData.fundingRequest.amount / 100
        } for their proposal in "${
          commonData.name
        }". See the proposal and vote.`,
        image: commonData.image || '',
  ...
  [EVENT_TYPES.FUNDING_REQUEST_ACCEPTED]: {
    ...
    notification: async ({proposalData, commonData}) => {
      return {
        ...
        body: `A funding proposal for $${
          proposalData.fundingRequest.amount / 100
        } was approved by "${commonData.name}".`,
        image: commonData.image || '',
        path: `ProposalScreen/${commonData.id}/${proposalData.id}`,
      };
    },
    ...
    email: ({
      ...
      const userTemplate = getFundingRequestAcceptedTemplate(
        cardMetadata?.billingDetails?.country,
        proposalData.fundingRequest.amount,
      );
      return [
        {
          to: userData.email,
          from:
            proposalData.fundingRequest.amount === 0
              ? env.mail.sender
              : env.mail.payoutEmail,
        ...
        },
        {
          ...
          emailStubs: {
            commonName: commonData.name,
            commonLink: Utils.getCommonLink(commonData.id),
            commonBalance: (commonData.balance / 100).toLocaleString('en-US', {
              style: 'currency',
              currency: 'USD',
            }),
            commonId: commonData.id,
  ...
  [EVENT_TYPES.FUNDING_REQUEST_ACCEPTED_INSUFFICIENT_FUNDS]: {
    ...
    email: ({user, proposal, common}): ISendTemplatedEmailData => ({
      ...
      emailStubs: {
        ...
        commonBalance: (common.balance / 100).toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
        }),
      },
  ...
  [EVENT_TYPES.MESSAGE_CREATED]: {
    ...
    data: async (messageId: string) => {
      const discussionMessage = (
        await getDiscussionMessageById(messageId)
      ).data();
      const commonId =
    ...
    notification: async ({sender, commonData, path}) => ({
      title: `New comment!`,
      body: `The member ${getNameString(sender)} commented in "${
        commonData.name
      }"`,
      image: commonData.image || '',
      path,
  ...
  [EVENT_TYPES.SUBSCRIPTION_PAYMENT_CONFIRMED]: {
    ...
    notification: async ({commonData, subscription}) =>
      subscription.charges === 1 ? memberAddedNotification(commonData) : null,
    email: ({
      subscription,
      user,
      card,
      ...
      emailStubs: {
        ...
        chargeAmount: (subscription.amount / 100).toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
        }),
      },
...
export default new (class Notification implements INotification {
  async send(
    ...
    const messageSent: admin.messaging.MessagingDevicesResponse =
      tokens.length > 0 &&
