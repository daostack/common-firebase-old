...
export const onPayoutCreated: IEventTrigger = async (eventObj) => {
  ...
  const proposal =
    payout.type === 'proposal'
      ? await proposalDb.getProposal(
          (payout as IProposalPayoutEntity).proposalId,
        )
      : null;

  const common = proposal ? await commonDb.get(proposal.commonId) : null;

  env.payouts.approvers.map(async (approver, index) => {
    const urlBase =
      process.env.NODE_ENV === 'dev' ? env.local : env.endpoints.base;
    ...
    await emailClient.sendTemplatedEmail({
      ...
      emailStubs: {
        ...
        common: common ? `${common.name} (${common.id})` : 'Independent Payout',

        bankDescription:
          wire.description || 'The bank account has no description',
        bank: wire.bank.bankName,

        payoutId: payout.id,
        amount: (payout.amount / 100).toLocaleString('en-US', {
          style: 'currency',
          currency: 'USD',
        }),
