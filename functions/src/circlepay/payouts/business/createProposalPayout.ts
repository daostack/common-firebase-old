...
type CreateProposalPayoutPayload = yup.InferType<
  typeof createProposalPayoutValidationSchema
>;

export const createProposalPayout = async (
  payload: CreateProposalPayoutPayload,
  ...
  if (proposal.state !== 'passed') {
    throw new CommonError(
      'Cannot create payout for proposals that are not in the passed state',
    );
  }
  ...
  if (activeOrCompletedPayoutForProposal.length) {
    logger.warn(
      'Payout was attempted to be created for proposal that has completed or pending payout',
    );

    throw new CommonError(
      `Cannot create payout for proposal with completed or pending payout`,
      {
        proposal,
        // If I just left it all it would return the security tokens
        activeOrCompletedPayoutForProposal: activeOrCompletedPayoutForProposal.map(
          (payout) => payout.id,
        ),
      },
    );
