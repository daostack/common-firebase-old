...
export const updatePayoutStatus = async (
  ...
  const {
    data: response,
  } = await externalRequestExecutor<ICircleGetPayoutResponse>(
    async () => {
      return (
        await axios.get<ICircleGetPayoutResponse>(
          `${circlePayApi}/payout/${payout.circlePayoutId}`,
          headers,
        )
      ).data;
    },
