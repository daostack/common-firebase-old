...
import {
  ICircleCreatePayoutPayload,
  ICircleCreatePayoutResponse,
} from '../../types';
...
export const executePayout = async (payout: IPayoutEntity): Promise<void> => {
  ...
  const {
    data: response,
  } = await externalRequestExecutor<ICircleCreatePayoutResponse>(
    async () => {
      ...
      return (
        await axios.post<ICircleCreatePayoutResponse>(
          `${circlePayApi}/payouts`,
          data,
          headers,
        )
      ).data;
