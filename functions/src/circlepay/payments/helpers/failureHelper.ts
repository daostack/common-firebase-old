...
const failureCodeDescription: {
  ...
  credit_card_not_allowed:
    'Issuer did not support using a credit card for payment',
  payment_denied:
...
const getFailureDescription = (
  responseCode: PaymentFailureResponseCodes = 'payment_failed',
): string => failureCodeDescription[responseCode];

export const processFailedPayment = (
  payment: ICirclePayment,
): IPaymentFailureReason => {
  if (payment.data.status !== 'failed') {
    logger.warn(
      'Trying to process failed payment that is not actually failed',
      {
        payment,
      },
    );
