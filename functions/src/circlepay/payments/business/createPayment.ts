...
import {
  ICircleCreatePaymentPayload,
  ICircleCreatePaymentResponse,
} from '../../types';
...
const createPaymentValidationSchema = yup.object({
  ...
  type: yup.string().oneOf(['one-time', 'subscription']).required(),

  encryptedData: yup.string().when('type', {
    is: 'one-time',
    then: yup.string(), //.required()
  }),

  keyId: yup.string().when('type', {
    is: 'one-time',
    then: yup.string(), //.required()
  }),
});
...
interface ICreatePaymentPayload
  extends yup.InferType<typeof createPaymentValidationSchema> {
  /**
...
export const createPayment = async (
  ...
  const circleData: ICircleCreatePaymentPayload = {
    ...
    idempotencyKey: payload.type === 'one-time' ? payload.proposalId : v4(), // @todo This will not work for the second subscription payment, so append the
  ...
  const {
    data: response,
  } = await externalRequestExecutor<ICircleCreatePaymentResponse>(
    async () => {
      ...
      return (
        await axios.post<ICircleCreatePaymentResponse>(
          `${circlePayApi}/payments`,
          circleData,
          headers,
        )
      ).data;
