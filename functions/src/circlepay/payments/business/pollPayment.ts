...
export const pollPayment = async (
  ...
  pollPaymentOptions?: IPollPaymentOptions,
  ...
  const pollFn = async (): Promise<ICirclePayment> => {
    return externalRequestExecutor<ICirclePayment>(
      async () => {
        return (
          await axios.get<ICirclePayment>(
            `${circlePayApi}/payments/${payment.circlePaymentId}`,
            headers,
          )
        ).data;
      },
  ...
  const circlePaymentObj = await poll<ICirclePayment>(
    pollFn,
    validateFn,
    options.interval,
    options.maxRetries,
