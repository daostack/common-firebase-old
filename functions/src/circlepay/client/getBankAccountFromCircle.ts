...
export const getBankAccountFromCircle = async (
  bankAccountId: string,
): Promise<ICircleGetBankAccountResponse> => {
  const headers = await getCircleHeaders();

  return await externalRequestExecutor<ICircleGetBankAccountResponse>(
    async () => {
      return (
        await axios.get<ICircleGetBankAccountResponse>(
          `${circlePayApi}/banks/wires/${bankAccountId}`,
          headers,
        )
      ).data;
    },
