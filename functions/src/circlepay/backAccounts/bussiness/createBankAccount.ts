...
import {
  ICircleCreateBankAccountPayload,
  ICircleCreateBankAccountResponse,
} from '../../cards/circleTypes';
  ...
  iban: yup
    ...
    .when('accountNumber', {
      ...
      then: yup
        .string()
        .required(
          'The IBAN is required field when there are not account number and routing number provided',
        ),
    ...
    .test({
      ...
      test: (value): boolean => {
        return isNullOrUndefined(value) || iban.isValidIBAN(value);
      },
    }),

  accountNumber: yup.string().when('billingDetails.country', {
  ...
  routingNumber: yup
    ...
    .when('iban', {
      ...
      then: yup
        .string()
        .required(
          'The routing number is required when the IBAN is not provided',
        ),
    })
...
type CreateBankAccountPayload = yup.InferType<
...
export const createBankAccount = async (
  ...
  const {
    data: response,
  } = await externalRequestExecutor<ICircleCreateBankAccountResponse>(
    async () => {
      ...
      return (
        await axios.post<ICircleCreateBankAccountResponse>(
          `${circlePayApi}/banks/wires`,
          data,
          headers,
        )
      ).data;
    },
    {
      ...
      userMessage:
        'Cannot create the bank account, because it was rejected by Circle',
    },
  );
