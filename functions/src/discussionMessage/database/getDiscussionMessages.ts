...
export const getDiscussionMessages = async (
  ...
  const discussionMessagesSnapshot = await getDiscussionMessagsSnapshot(
    discussionId,
    limit,
    startDoc,
  );
  return discussionMessagesSnapshot.map(
    (message) => message.data() as IDiscussionMessage,
  );
};

export const getDiscussionMessagsSnapshot = async (
  discussionId: string,
  limit = 1,
  startDoc = null,
): Promise<any> => {
  ...
  return ((await discussionMessageQuery
    .limit(limit)
    .get()) as QuerySnapshot<IDiscussionMessage>).docs;
};
