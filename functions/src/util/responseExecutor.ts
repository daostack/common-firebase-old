...
interface IResponseExecutor {
  (
    action: IResponseExecutorAction,
    payload: IResponseExecutorPayload,
  ): Promise<void>;
}

export const responseExecutor: IResponseExecutor = async (
  action,
  {req, res, next, successMessage},
): Promise<void> => {
  try {
    ...
    logger.debug(
      "An error occurred while executing the response executor's action",
      {
        error: e,
      },
    );
