...
export const updateProposal = async (
  proposal: WithRequired<IProposalEntity, 'id'> | IProposalEntity,
): Promise<WithRequired<IProposalEntity, 'id'>> => {
  const proposalDoc = {
    ...proposal,
  ...
  await ProposalsCollection.doc(proposalDoc.id).update(proposalDoc);
