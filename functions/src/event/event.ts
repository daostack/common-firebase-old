 ...
 */
const limitRecipients = async (
  discussionOwner: string,
  discussionId: string,
  discussionMessageOwner: string,
): Promise<string[]> => {
  let users = [],
    lastDoc = null,
    didBreak = false;
  ...
  do {
    ...
    const messages = await discussionMessageDb.getDiscussionMessagsSnapshot(
      discussionId,
      Notifications.messageLimit,
      lastDoc,
    );
    // the last doc from which to start counting the next batch of messages
    lastDoc = messages[messages.length - 1];
    users = messages.map(
      (message) => (message.data() as IDiscussionMessage).ownerId,
    );
    ...
    userFilter.length <= Notifications.maxNotifications &&
...
const handleUserFilter = (userIDs: string[], userFilter: string[]): boolean => {
  ...
  for (
    let i = 1, limitCounter = userFilter.length;
    i < userIDs.length && limitCounter <= Notifications.maxNotifications;
    i++
  ) {
...
export const eventData: Record<string, IEventData> = {
  [EVENT_TYPES.COMMON_CREATED]: {
    eventObject: async (commonId: string): Promise<any> =>
      await commonDb.get(commonId),
  ...
  [EVENT_TYPES.FUNDING_REQUEST_CREATED]: {
    eventObject: async (proposalId: string): Promise<any> =>
      await proposalDb.getProposal(proposalId),
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  ...
  [EVENT_TYPES.MESSAGE_CREATED]: {
    eventObject: async (discussionMessageId: string): Promise<any> =>
      (await getDiscussionMessageById(discussionMessageId)).data(),
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
  ...
  [EVENT_TYPES.SUBSCRIPTION_CANCELED_BY_PAYMENT_FAILURE]: {
    eventObject: async (subscriptionId: string): Promise<any> =>
  ...
  [EVENT_TYPES.SUBSCRIPTION_PAYMENT_CONFIRMED]: {
    eventObject: async (paymentId: string): Promise<any> =>
      await paymentDb.get(paymentId),
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
