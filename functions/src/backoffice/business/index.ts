  ...
  for (const key in data) {
    ...
    if (data.hasOwnProperty(key)) {
      ...
      if (data[key].subscription) {
        ...
        cells.push(
          `${date(new Date(data[key].subscription.createdAt.toDate()))}`,
        );
        cells.push(
          `${date(new Date(data[key].subscription.updatedAt.toDate()))}`,
        );
      } else {
        cells.push('');
        cells.push('');
...
export async function filCircleBalanceSheet(): Promise<any> {
  ...
  const values = [['Account', 'Available', 'Unsettled', 'Date']];
  for (let i = 0; i < data.available.length; i++) {
    const cells = [];
    cells.push(i + 1);
    if (data.available[i]) cells.push(parseFloat(data.available[i].amount));
    else cells.push(0);
    if (data.unsettled[i]) cells.push(parseFloat(data.unsettled[i].amount));
    else cells.push(0);
    cells.push(date());
    values.push(cells);
  }
...
export async function fillCommonBalanceSheet(): Promise<any> {
  ...
  const values = [['Common id', 'Common name', 'Balance', 'Date']];
  for (const key in data) {
    // eslint-disable-next-line no-prototype-builtins
    if (data.hasOwnProperty(key)) {
      const cells = [];
